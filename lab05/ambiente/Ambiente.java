...
public class Ambiente {
    ...
    /**
     * Retorna uma representação em String do ambiente em um nível Z específico
     * @param nivelZ O nível Z do ambiente a ser exibido
     * @return String representando o ambiente no nível Z especificado
     */
    public String toStringNivelZ(int nivelZ) {
        StringBuilder sb = new StringBuilder();
        sb.append("Ambiente no nível Z=").append(nivelZ).append("\n");
        
        // Adiciona números das colunas no topo
        sb.append("   ");
        for (int x = 0; x < largura; x++) {
            sb.append(x).append(" ");
        }
        ...
        // Para cada linha
        for (int y = altura - 1; y >= 0; y--) {
            // Adiciona número da linha
            sb.append(String.format("%2d ", y));
            
            // Para cada coluna
            for (int x = 0; x < largura; x++) {
                boolean posicaoOcupada = false;
                char representacao = '.';  // Representa espaço vazio

                // Verifica todas as entidades nesta posição
                for (Entidade entidade : entidades) {
                    if (entidade.getX() == x && entidade.getY() == y && entidade.getZ() == nivelZ) {
                ...
                sb.append(representacao).append(" ");
            }
            sb.append("\n");
        }
        return sb.toString();
